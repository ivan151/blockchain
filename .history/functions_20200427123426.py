...
def check_integrity():
    ...
    # compare results
    results = []
    files = int_and_sort(os.listdir(blockchain_dir))[:-1]
    for filename in reversed(files):
        actual_hash = get_hash(str(filename))
        next_file = str(filename + 1)
        next_file_data = json.load(open(blockchain_dir + str(next_file), 'rb'))
        correct_hash = next_file_data['hash']
        if actual_hash == correct_hash:
            result = 'ok'
        else:
            result = 'corrupted'
        results.append({next_file: result})
    return results
