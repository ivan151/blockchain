...
def edit(num):
    if request.method == 'POST':
        ...
        changed_block = request.form['edit']
        block = json.load(changed_block,'rb')
        write_block(from_who=block['from'],  to_whom=block['to'], amount=block['amount'], prev_hash=block['prev_hash'])
