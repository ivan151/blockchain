...
from functions import node_dirs, write_block, write_block_to_nodes, genesis, change_block, get_hash, int_and_sort
from functions import blockchain_dir, nodes_route, files
import random
import uuid
import json
import os
...
def private():
    ...
    if request.method == 'GET':
        ...
        nodes_numbers = []
        for i in range(1,7):
            nodes_numbers.append(request.form[f"node{i}]["value"])
...
def edit(num):
    ...
    if request.method == 'GET':
        ...
        return render_template('edit.html', edited='BLOCK IS CHANGED!', number=num)
