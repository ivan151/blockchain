...
def transaction():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('private',sen=sender_public_key, rec=receiver_public_key, am = amount ))
    elif request.method == 'GET':
        return render_template('transactions.html', users=users)


@app.route('/transaction/private/{sen}{res}{am}/', methods=['GET', 'POST'])
def private():
    amount = session.get('amount')
    sender_public_key = session.get('sender_public_key')
    receiver_public_key = session.get('receiver_public_key')
    sender = User.query.filter(User.name == sender_public_key).first()
    print(amount)
