...
def transaction():
    ...
    if request.method == 'POST':
        ...
        sender_private_key = User.query.filter_by(public_key=sender_public_key).first()
        print(sender_public_key, sender_private_key, receiver_public_key,amount)
        return render_template('transactions.html', sender=sender_public_key, receiver=receiver_public_key, amount=amount, users=users)
    else:
...
def private():
    if request.method == 'GET':
        sender_public_key = request.url.get('from')
        receiver_public_key = request.form.get('to')
        amount = request.form.get('amount')
